class FakerApi {
    ...
    _setUser(user) {
        ...
        if (!user.id) {
            ...
            users = users.map((postMap) =>
                postMap.id === user.id ? user : postMap
            );
        }
    ...
    _setPost(post) {
        ...
        if (!post.id) {
            ...
            posts = posts.map((postMap) =>
                postMap.id === post.id && postMap.user_id === auth.id
                    ? post
                    : postMap
            );
        }
    ...
    _removePost(post_id) {
        ...
        const posts = this._allPosts().filter(
            (postMap) => postMap.id !== post_id
        );
        this._insertPosts(posts);
    }
...
export default window.FakerApi = new FakerApi();
